...
class MongoServer {
  ...
  initMongo(servers) {
    ...
    if (serverList.length > 1) {
      this.serverCluster = new mongo.ReplSet(serverList);
    } else {
      this.serverCluster = serverList[0];
    }
  }

  openDb(dbName, cb) {
    new mongo.Db(dbName, this.serverCluster, {safe: true}).open(cb);
  }
