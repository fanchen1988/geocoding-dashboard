...
import {getGeoSumTaskInfo} from './run';
...
function attachTaskInfo(record, cb) {
  waterfall([
    (callback) => {
      getGeoSumTaskInfo(record.dataset, callback);
    }
  ], (err, taskInfo) => {
    taskInfo = taskInfo || {};
    record.taskId = taskInfo.taskId;
    record.taskStatus = taskInfo.taskStatus;
    cb(null, record);
  });
}
...
export function getGeosummarizerData(cb) {
  waterfall([
    ...
    (records, callback) => {
      map(records, attachTaskInfo, callback);
    },
